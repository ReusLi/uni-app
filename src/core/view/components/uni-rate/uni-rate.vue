<template><VUniView class="uni-rate">
		<VUniView :key="index" :style="{ marginLeft: margin + 'px' }" @click="_onClick(index)" class="uni-rate__icon" v-for="(star, index) in stars">
			<VUniUniIcons :color="color" :size="size" :type="isFill ? 'star-filled' : 'star'">
			<!-- #ifdef APP-NVUE -->
			<VUniView :style="{ width: star.activeWitch.replace('%','')*size/100+'px'}" class="uni-rate__icon-on">
				<VUniUniIcons style="text-align: left;" :color="activeColor" :size="size" type="star-filled">
			</VUniUniIcons></VUniView>
			<!-- #endif -->
			<!-- #ifndef APP-NVUE -->
			<VUniView :style="{ width: star.activeWitch,top:-size/2+'px' }" class="uni-rate__icon-on">
				<VUniUniIcons :color="activeColor" :size="size" type="star-filled">
			</VUniUniIcons></VUniView>
			<!-- #endif -->
		</VUniUniIcons></VUniView>
	</VUniView>
