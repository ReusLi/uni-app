<template><VUniView class="uni-swiper__warp">
		<VUniSlot>
		<VUniView v-if="mode === 'default'" :style="{'bottom':dots.bottom + 'px'}" class="uni-swiper__dots-box" key="default">
			<VUniView v-for="(item,index) in info" :style="{
		...
		<VUniView v-if="mode === 'dot'" :style="{'bottom':dots.bottom + 'px'}" class="uni-swiper__dots-box" key="dot">
			<VUniView v-for="(item,index) in info" :style="{
		...
		<VUniView v-if="mode === 'round'" :style="{'bottom':dots.bottom + 'px'}" class="uni-swiper__dots-box" key="round">
			<VUniView v-for="(item,index) in info" :class="[index === current&&'uni-swiper__dots-long']" :style="{
		...
		<VUniView v-if="mode === 'nav'" key="nav" :style="{'background-color':dotsStyles.backgroundColor,'bottom':'0'}" class="uni-swiper__dots-box uni-swiper__dots-nav">
			<VUniText :style="{'color':dotsStyles.color}" class="uni-swiper__dots-nav-item">{{ (current+1)+"/"+info.length +' ' +info[current][field] }}</VUniText>
		</VUniView>
		<VUniView v-if="mode === 'indexes'" key="indexes" :style="{'bottom':dots.bottom + 'px'}" class="uni-swiper__dots-box">
			<VUniView v-for="(item,index) in info" :style="{
        'width':dots.width + 'px','height':dots.height +'px' ,'color':index === current?dots.selectedColor:dots.color,'background-color':index !== current?dots.backgroundColor:dots.selectedBackgroundColor,'border':index !==current ? dots.border:dots.selectedBorder}" :key="index" class="uni-swiper__dots-item uni-swiper__dots-indexes"><VUniText class="uni-swiper__dots-indexes-text">{{ index+1 }}</VUniText></VUniView>
		</VUniView>
	</VUniView>
